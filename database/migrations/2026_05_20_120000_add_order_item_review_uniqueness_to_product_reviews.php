...
return new class extends Migration
{
    ...
    {
        // Ensure FK-supporting standalone indexes exist before dropping unique index.
        $this->ensureIndex('product_reviews', 'product_reviews_order_id_index', ['order_id']);
        ...
        $this->ensureIndex('product_reviews', 'product_reviews_order_item_id_index', ['order_item_id']);
        ...
        Schema::table('product_reviews', function (Blueprint $table) {
            ...
            if (!$this->indexExists('product_reviews', 'product_reviews_user_order_item_product_unique')) {
                $table->unique(
                    ['user_id', 'order_item_id', 'product_id'],
                    'product_reviews_user_order_item_product_unique'
                );
            }
        });
    }
    ...
    public function down(): void
    {
        Schema::table('product_reviews', function (Blueprint $table) {
            if ($this->indexExists('product_reviews', 'product_reviews_user_order_item_product_unique')) {
                $table->dropUnique('product_reviews_user_order_item_product_unique');
            }

            if (!$this->indexExists('product_reviews', 'product_reviews_order_product_user_unique')) {
                $table->unique(
                    ['order_id', 'product_id', 'user_id'],
                    'product_reviews_order_product_user_unique'
                );
            }
        });
    }
