...
@section('title', $article->meta_title ?: $article->title . __('blog_page.title_suffix'))
@section('meta_description', $article->meta_description ?: $article->excerpt)

@php
    $ogImageUrl = $article->cover_image
        ? route('article.image', ['filename' => basename($article->cover_image)])
        : asset('images/official-logo.png');
@endphp
@push('head')
    <!-- Open Graph / Facebook -->
    ...
    <meta property="og:locale" content="{{ __('blog_page.og_locale') }}">
    @if($article->author)
            ...
            <ol class="flex items-center space-x-4">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('blog.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">{{ __('Berita') }}</a>
                    </div>
                </li>
                <li>
                    <div class="flex items-center">
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <article class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="relative">
            ...
            <div class="absolute bottom-6 left-6 right-6">
                ...
                <div class="flex items-center text-white text-sm">
                    ...
                    {{ number_format($article->views_count) }} {{ __('tontonan') }}
                    <span class="mx-2">{{ __('•') }}</span>
                    <span>{{ __('blog_page.read_time', ['minutes' => ceil(str_word_count(strip_tags($article->content)) / 200)]) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="p-8 md:p-12">
            ...
            <!-- AdSense Ad Section - Below Article Content -->
            ...
            <!-- Social Sharing -->
            @php
                $whatsappShareText = ($article->title ?? '') . ' ' . url()->current();
                $whatsappShareUrl = 'https://wa.me/?text=' . rawurlencode($whatsappShareText);
            @endphp
            <div class="mt-8 pt-8 border-t border-gray-200">
                <h3 class="text-sm font-medium text-gray-900 mb-4">{{ __('Kongsi artikel ini') }}</h3>
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <a href="{{ $whatsappShareUrl }}"
                       target="_blank" rel="noopener noreferrer"
                       class="flex items-center justify-center w-10 h-10 rounded-full transition-opacity hover:opacity-90 flex-shrink-0"
                       style="background-color: #25D366; color: #fff;"
                       aria-label="{{ __('blog_page.share_whatsapp_aria') }}"
                       title="{{ __('blog_page.share_whatsapp') }}">
    ...
    <div class="bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="text-center mb-12">
                <h2 class="text-3xl font-bold text-gray-900 mb-4">{{ __('Artikel Berkaitan') }}</h2>
                <p class="text-gray-600">{{ __('Lebih banyak cerita dari komuniti Arsenal') }}</p>
            </div>
                    ...
                    <article class="bg-white rounded-xl shadow-lg overflow-hidden group hover:shadow-xl transition-shadow">
                        <div class="relative">
                            @if($relatedArticle->cover_image)
                                <img src="{{ route('article.image', basename($relatedArticle->cover_image)) }}" 
            ...
            <div class="text-center mt-12">
                <a href="{{ route('blog.index') }}" 
                   class="bg-arsenal hover:bg-arsenal text-white px-8 py-3 rounded-lg font-medium transition-colors">{{ __('Lihat Semua Artikel') }}</a>
            </div>
...
function copyToClipboard() {
    navigator.clipboard.writeText(window.location.href).then(function() {
        ...
        alert(window.clientMessages.link_copied);
    }, function(err) {
