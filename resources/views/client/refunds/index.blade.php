...
@section('title', __('Permohonan Refund Saya - MyGooners'))
@section('meta_description', __('Lihat semua permohonan refund anda di MyGooners.'))
...
<div class="bg-gray-50 border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span class="text-gray-900 font-medium">{{ __('Permohonan Refund') }}</span>
        </nav>
    </div>
</div>
...
<div class="bg-white border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('Permohonan Refund Saya') }}</h1>
                <p class="text-gray-600 mt-1">{{ __('Lihat dan uruskan semua permohonan refund anda') }}</p>
            </div>
            
            <a href="{{ route('checkout.orders') }}" 
               class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg font-medium transition-colors">{{ __('Lihat Pesanan') }}</a>
        </div>
    </div>
</div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex items-start justify-between">
            <div class="flex items-start space-x-3 flex-1">
                ...
                <div class="flex-1">
                    <h3 class="text-sm font-medium text-blue-900 mb-1">{{ __('Maklumat Penting Mengenai Refund') }}</h3>
                    <div class="text-sm text-blue-800 space-y-1">
                        <p>{{ __('•') }}<strong>{{ __('Tempoh Refund:') }}</strong> {{ __('Refund hanya boleh dimohon dalam tempoh 3 hari selepas pesanan diterima') }}</p>
                        <p>{{ __('•') }}<strong>{{ __('Jenis Refund:') }}</strong> {{ __('Return & Refund') }}</p>
                        <p>{{ __('•') }}<strong>{{ __('Bukti:') }}</strong> {{ __('Setiap permohonan memerlukan 3 gambar sebagai bukti') }}</p>
                        <p>{{ __('•') }}<strong>{{ __('Maklumat Bank:') }}</strong> {{ __('Pastikan maklumat bank anda tepat untuk pemprosesan refund') }}</p>
                        <p>{{ __('•') }}<strong>{{ __('Masa Semakan:') }}</strong> {{ __('Admin akan menyemak permohonan dalam masa 24-48 jam') }}</p>
                    </div>
                </div>
            </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                        <div class="p-6">
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-900">
                                        {{ trans('account_page.refund_number', ['id' => $refund->id, 'order_number' => $refund->order->order_number]) }}
                                    </h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                                ...
                                <div>
                                    <p class="text-sm text-gray-600">{{ __('Akaun') }}</p>
                            ...
                            <!-- Refund Reason -->
                            <div class="mb-4">
                                <p class="text-sm text-gray-600 mb-1">{{ __('Sebab Refund:') }}</p>
                            ...
                            @if($refund->status === 'rejected' && $refund->rejection_reason)
                                <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div>
                                            <h4 class="text-sm font-medium text-red-900">{{ __('Sebab Penolakan') }}</h4>
                            ...
                            @if($refund->status === 'approved' && $refund->admin_notes)
                                <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div>
                                            <h4 class="text-sm font-medium text-green-900">{{ __('Nota Admin') }}</h4>
                                    ...
                                    @elseif($refund->status === 'approved')
                                        <div class="flex flex-col space-y-2">
                                            <span class="text-sm text-blue-600">
                                                ...
                                                {{ __('Refund diluluskan') }}
                                            </span>
                                            <span class="text-xs text-gray-600">{{ __('Klik "Lihat Detail" untuk berikan maklumat bank dan tracking') }}</span>
                                        </div>
                                    @elseif($refund->status === 'processing')
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('Tiada Permohonan Refund') }}</h3>
                <p class="mt-1 text-sm text-gray-500">
                    {{ __('Anda belum membuat sebarang permohonan refund.') }}
                    <a href="{{ route('checkout.orders') }}" class="text-red-600 hover:text-red-500 font-medium">{{ __('Lihat pesanan anda') }}</a>
                    {{ __('untuk memohon refund jika diperlukan.') }}
                </p>
            </div>
        @endif
    </div>
