...
@section('title', __('Edit Alamat Bil - MyGooners'))
@section('meta_description', __('Edit alamat bil di MyGooners.'))
...
<div class="bg-gray-50 border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span class="text-gray-900 font-medium">{{ __('Edit Alamat') }}</span>
        </nav>
    </div>
</div>
...
<div class="bg-white border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('Edit Alamat Bil') }}</h1>
                <p class="text-gray-600 mt-1">{{ __('Kemas kini maklumat alamat bil anda') }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <form method="POST" action="{{ route('addresses.billing.update', $billingDetail) }}" class="p-6">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                              placeholder="{{ __('Masukkan alamat lengkap') }}">{{ old('address', $billingDetail->address) }}</textarea>
                    @error('address')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200 mt-8">
                ...
                <button type="submit" 
                        class="bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded-lg font-medium transition-colors">{{ __('Kemas Kini Alamat Bil') }}</button>
            </div>
        </form>
    </div>
</div>
