...
<div x-data="{ showIdModal: false, showSelfieModal: false, showProfileModal: false }" x-effect="document.body.classList.toggle('overflow-hidden', showProfileModal)" class="min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto py-4 sm:py-6 lg:py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex-1">
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Maklumat Profil</h1>
                    <p class="text-sm sm:text-base text-gray-600 leading-relaxed">Maklumat lengkap profil dan akaun anda</p>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->phone ?: 'Tiada maklumat' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->location ?: 'Tiada maklumat' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Bio</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->bio ?: 'Tiada maklumat' }}</p>
                    </div>
                    @if($user->is_seller)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tahun Pengalaman</label>
                            <p class="text-gray-900 text-sm sm:text-base">{{ $user->years_experience ? $user->years_experience . ' tahun' : 'Tiada maklumat' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Kemahiran</label>
                            <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->skills ?: 'Tiada maklumat' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Kawasan Perkhidmatan</label>
                            <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->service_areas ?: 'Tiada maklumat' }}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    <div>
                        ...
                        <p class="text-gray-900 font-medium text-sm sm:text-base break-words">{{ $user->business_name ?: 'Tiada maklumat' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Perniagaan</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->business_type ?: 'Tiada maklumat' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">No. Pendaftaran Perniagaan</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->business_registration ?: 'Tiada maklumat' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Perniagaan</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->business_address ?: 'Tiada maklumat' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Kawasan Operasi</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->operating_area ?: 'Tiada maklumat' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Laman Web/Sosial Media</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->website ?: 'Tiada maklumat' }}</p>
                    </div>
                </div>
            </div>
            @else
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    <div>
                        ...
                        <p class="text-gray-900 font-medium text-sm sm:text-base">Akaun Biasa</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-gray-900 text-sm sm:text-base">{{ $user->last_login ? $user->last_login->format('d M Y H:i') : 'Tiada maklumat' }}</p>
                    </div>
                </div>
            </div>
            @endif
                ...
                @if($user->id_document)
                    <div class="border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center justify-between mb-3">
                            <h4 class="font-semibold text-gray-900 text-sm sm:text-base">Kad Pengenalan / Sijil / Lesen Perniagaan</h4>
        ...
        <div class="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl p-4 sm:p-6">
            ...
            <p class="text-yellow-700 mb-4 text-sm sm:text-base">Daftar sebagai penjual untuk mula menjual produk dan perkhidmatan kepada komuniti Gooners.</p>
    ...
    <div x-show="showIdModal" x-transition:enter="transition ease-out duration-200" x-transition:leave="transition ease-in duration-150" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4" x-trap="showIdModal" @keydown.window.escape="showIdModal = false" aria-modal="true" role="dialog">
        <div @click.away="showIdModal = false" class="bg-white rounded-lg shadow-lg max-w-4xl w-full p-4 sm:p-6 relative">
            ...
            <h3 class="text-lg sm:text-xl font-bold mb-4">Kad Pengenalan / Sijil / Lesen Perniagaan</h3>
    ...
    <div x-show="showSelfieModal" x-transition:enter="transition ease-out duration-200" x-transition:leave="transition ease-in duration-150" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4" x-trap="showSelfieModal" @keydown.window.escape="showSelfieModal = false" aria-modal="true" role="dialog">
        <div @click.away="showSelfieModal = false" class="bg-white rounded-lg shadow-lg max-w-4xl w-full p-4 sm:p-6 relative">
            ...
            <h3 class="text-lg sm:text-xl font-bold mb-4">Selfie Bersama ID</h3>
            <div class="flex justify-center">
            ...
            <div class="flex-1 overflow-y-auto p-4 sm:p-6 min-h-0">
                <form id="profile-update-form" method="POST" action="{{ route('dashboard.update_profile') }}" enctype="multipart/form-data" class="space-y-4 sm:space-y-6">
                    ...
                    @if($user->is_seller)
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">Nama Perniagaan</label>
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">Jenis Perniagaan</label>
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">No. Pendaftaran Perniagaan</label>
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">Kemahiran / Tag / Kepakaran</label>
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">Kawasan Perkhidmatan / Liputan</label>
