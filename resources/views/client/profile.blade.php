...
@section('title', __('Maklumat Profil - MyGooners'))

@section('content')
<div x-data="{ showIdModal: false, showSelfieModal: false, showProfileModal: false }" x-effect="document.body.classList.toggle('overflow-hidden', showProfileModal)" class="min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto py-4 sm:py-6 lg:py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex-1">
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{{ __('Maklumat Profil') }}</h1>
                    <p class="text-sm sm:text-base text-gray-600 leading-relaxed">{{ __('Maklumat lengkap profil dan akaun anda') }}</p>
                </div>
                ...
                <div class="text-center sm:text-left flex-1">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                        @if($user->is_seller)
                            ...
                            @endif
                        @else
                            <span class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 text-sm font-semibold">
                                <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                </svg>
                                {{ __('Akaun Biasa') }}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 mb-6 sm:mb-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->phone ?: __('Tiada maklumat') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Lokasi') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->location ?: __('Tiada maklumat') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Bio') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->bio ?: __('Tiada maklumat') }}</p>
                    </div>
                    @if($user->is_seller)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Tahun Pengalaman') }}</label>
                            <p class="text-gray-900 text-sm sm:text-base">{{ $user->years_experience ? trans('account_page.years_experience', ['count' => $user->years_experience]) : __('Tiada maklumat') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Kemahiran') }}</label>
                            <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->skills ?: __('Tiada maklumat') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Kawasan Perkhidmatan') }}</label>
                            <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->service_areas ?: __('Tiada maklumat') }}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    <div>
                        ...
                        <p class="text-gray-900 font-medium text-sm sm:text-base break-words">{{ $user->business_name ?: __('Tiada maklumat') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Jenis Perniagaan') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->business_type ?: __('Tiada maklumat') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('No. Pendaftaran Perniagaan') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->business_registration ?: __('Tiada maklumat') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Alamat Perniagaan') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->business_address ?: __('Tiada maklumat') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Kawasan Operasi') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->operating_area ?: __('Tiada maklumat') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Laman Web/Sosial Media') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base break-words">{{ $user->website ?: __('Tiada maklumat') }}</p>
                    </div>
                </div>
            </div>
            @else
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Status Akaun') }}</label>
                        <p class="text-gray-900 font-medium text-sm sm:text-base">{{ __('Akaun Biasa') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Tarikh Pendaftaran') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base">{{ $user->created_at->format('d M Y') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Kemaskini Terakhir') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base">{{ $user->updated_at->format('d M Y') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Log Masuk Terakhir') }}</label>
                        <p class="text-gray-900 text-sm sm:text-base">{{ $user->last_login ? $user->last_login->format('d M Y H:i') : __('Tiada maklumat') }}</p>
                    </div>
                </div>
            </div>
            @endif
                ...
                @if($user->id_document)
                    <div class="border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center justify-between mb-3">
                            <h4 class="font-semibold text-gray-900 text-sm sm:text-base">{{ __('Kad Pengenalan / Sijil / Lesen Perniagaan') }}</h4>
                        ...
                        <div class="bg-gray-50 rounded-lg p-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-600">{{ __('Dokumen telah dimuat naik') }}</span>
                            </div>
                        </div>
                    </div>
                @endif
        ...
        <div class="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl p-4 sm:p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-lg font-bold text-yellow-800">{{ __('Jana Pendapatan dengan Menjadi Penjual') }}</h3>
            </div>
            <p class="text-yellow-700 mb-4 text-sm sm:text-base">{{ __('Daftar sebagai penjual untuk mula menjual produk dan perkhidmatan kepada komuniti Gooners.') }}</p>
            <a href="{{ route('dashboard') }}" class="inline-flex items-center justify-center gap-2 bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-3 rounded-lg font-semibold transition-colors text-sm sm:text-base">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if($user->is_seller)
    <!-- ID Document Modal -->
    <div x-show="showIdModal" x-transition:enter="transition ease-out duration-200" x-transition:leave="transition ease-in duration-150" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4" x-trap="showIdModal" @keydown.window.escape="showIdModal = false" aria-modal="true" role="dialog">
        <div @click.away="showIdModal = false" class="bg-white rounded-lg shadow-lg max-w-4xl w-full p-4 sm:p-6 relative">
            <button @click="showIdModal = false" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 text-2xl" aria-label="{{ __('Tutup') }}">&times;</button>
            <h3 class="text-lg sm:text-xl font-bold mb-4">{{ __('Kad Pengenalan / Sijil / Lesen Perniagaan') }}</h3>
                    ...
                    @elseif(Str::endsWith(strtolower($idDoc), ['.pdf']))
                        <div class="w-full h-96 bg-gray-100 rounded flex items-center justify-center">
                            <div class="text-center">
                                ...
                                <p class="text-gray-600">{{ __('PDF Document') }}</p>
                                <a href="{{ route('seller.document', ['filename' => basename($idDoc)]) }}" target="_blank" class="text-blue-600 hover:text-blue-800 mt-2 inline-block">{{ __('Buka PDF') }}</a>
                            </div>
                        </div>
                    @endif
                @endif
    ...
    <!-- Selfie Modal -->
    <div x-show="showSelfieModal" x-transition:enter="transition ease-out duration-200" x-transition:leave="transition ease-in duration-150" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4" x-trap="showSelfieModal" @keydown.window.escape="showSelfieModal = false" aria-modal="true" role="dialog">
        <div @click.away="showSelfieModal = false" class="bg-white rounded-lg shadow-lg max-w-4xl w-full p-4 sm:p-6 relative">
            <button @click="showSelfieModal = false" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 text-2xl" aria-label="{{ __('Tutup') }}">&times;</button>
            <h3 class="text-lg sm:text-xl font-bold mb-4">{{ __('Selfie Bersama ID') }}</h3>
            <div class="flex justify-center">
            ...
            <div class="flex-1 overflow-y-auto p-4 sm:p-6 min-h-0">
                <form id="profile-update-form" method="POST" action="{{ route('dashboard.update_profile') }}" enctype="multipart/form-data" class="space-y-4 sm:space-y-6">
                    ...
                    @if($user->is_seller)
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">{{ __('Nama Perniagaan') }}</label>
                            <input type="text" name="business_name" class="w-full border border-gray-300 rounded-lg px-3 py-3 sm:py-2 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ old('business_name', $user->business_name) }}">
                        </div>
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">{{ __('Jenis Perniagaan') }}</label>
                            <input type="text" name="business_type" class="w-full border border-gray-300 rounded-lg px-3 py-3 sm:py-2 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ old('business_type', $user->business_type) }}">
                        </div>
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">{{ __('No. Pendaftaran Perniagaan') }}</label>
                            <input type="text" name="business_registration" class="w-full border border-gray-300 rounded-lg px-3 py-3 sm:py-2 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ old('business_registration', $user->business_registration) }}">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">{{ __('Kemahiran / Tag / Kepakaran') }}</label>
                        ...
                        <div>
                            <label class="block text-sm sm:text-base font-medium text-gray-700 mb-2">{{ __('Kawasan Perkhidmatan / Liputan') }}</label>
                            <input type="text" name="service_areas" class="w-full border border-gray-300 rounded-lg px-3 py-3 sm:py-2 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ old('service_areas', $user->service_areas) }}">
                        </div>
                        <div class="col-span-2">
