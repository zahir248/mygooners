...
@section('title', __('Checkout Langsung - MyGooners'))
@section('meta_description', __('Selesaikan pembelian anda secara langsung di MyGooners.'))
...
<div class="bg-gray-50 border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span class="text-gray-900 font-medium">{{ __('Checkout Langsung') }}</span>
        </nav>
    </div>
</div>
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
                    ...
                    <div class="p-6">
                        <textarea name="notes" rows="3" 
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors"
                                  placeholder="{{ __('Tambah nota khas untuk pesanan anda...') }}">{{ old('notes') }}</textarea>
                        @error('notes')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-start space-x-4 mb-4">
                        ...
                        <div class="text-right">
                            ...
                            @if($hasSale)
                                ...
                                <p class="text-xs text-green-600">{{ __('client_ui.save_amount', ['amount' => number_format(($originalPrice - $finalPrice) * $quantity, 2)]) }}</p>
                            @else
