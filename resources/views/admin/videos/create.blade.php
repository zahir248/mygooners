...
<div class="px-4 sm:px-6 lg:px-8 py-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Cipta Video Baharu') }}</h1>
            <p class="mt-2 text-sm text-gray-700">{{ __('Tambah dan terbitkan video Arsenal baharu') }}</p>
        </div>
...
<form action="{{ route('admin.videos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="px-6 py-4 space-y-6">
            ...
            <div>
                <label for="description" class="block text-sm font-medium text-gray-700 mb-2">
                    {{ trans('admin_page.video_description') }} <span class="text-red-500">*</span>
                </label>
                <textarea name="description" 
                          ...
                          placeholder="{{ __('Tulis deskripsi video anda di sini...') }}">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div id="image-preview" class="mt-4 hidden">
                    <p class="text-sm font-medium text-gray-700 mb-2">{{ __('Pratonton Thumbnail:') }}</p>
                ...
                <p class="mt-1 text-sm text-gray-500">{{ __('Pilihan: Jika tidak dimuat naik, thumbnail YouTube akan digunakan secara automatik') }}</p>
                @error('thumbnail')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- Live Video Preview -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('Pratonton Video Langsung') }}</label>
                <div id="video-preview-container" class="aspect-video bg-gray-100 rounded-lg overflow-hidden border border-gray-300">
                    <div id="video-placeholder" class="flex items-center justify-center h-full">
                        <div class="text-center">
                            ...
                            <p class="text-gray-500">{{ __('Masukkan ID video YouTube untuk melihat pratonton') }}</p>
                        </div>
                    </div>
                ...
                <p class="mt-2 text-sm text-gray-500">
                    <span id="youtube-link" class="hidden">
                        <a href="#" target="_blank" class="text-red-600 hover:text-red-800">{{ __('Lihat di YouTube →') }}</a>
                    </span>
                </p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="status" 
                        ...
                        <option value="draft" {{ old('status') === 'draft' ? 'selected' : '' }}>{{ __('Simpan sebagai Draf') }}</option>
                        <option value="published" {{ old('status') === 'published' ? 'selected' : '' }}>{{ __('Terbitkan Sekarang') }}</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
    ...
    <div class="flex justify-end space-x-3">
        ...
        <button type="submit" 
                class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">{{ __('Cipta Video') }}</button>
    </div>
</form>
@endsection
