...
@section('title', __('Cipta Perkhidmatan'))

@section('content')
<div class="px-4 sm:px-6 lg:px-8 py-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Cipta Perkhidmatan Baharu') }}</h1>
            <p class="mt-2 text-sm text-gray-700">{{ __('Isi maklumat untuk perkhidmatan baharu') }}</p>
        </div>
...
<form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white shadow rounded-lg">
        ...
        <div class="px-6 py-4 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="tags" id="tags" value="{{ old('tags') }}" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-red-500 focus:border-red-500 @error('tags') border-red-500 @enderror" placeholder="{{ __('coaching, arsenal, bola') }}">
                @error('tags')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="flex justify-end">
        <button type="submit" class="px-6 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 text-sm font-medium">{{ __('Cipta Perkhidmatan') }}</button>
    </div>
</form>
