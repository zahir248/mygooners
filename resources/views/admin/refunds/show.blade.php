...
@section('title', __('Butiran Bayaran Balik - Panel Pentadbir'))
...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('Butiran Bayaran Balik') }}</h1>
                <p class="text-gray-600 mt-1">Bayaran Balik #{{ $refund->id }} - {{ $refund->order->order_number }}</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Status Pesanan') }}</p>
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-900">{{ $refund->order->delivered_at ? $refund->order->delivered_at->format('d/m/Y H:i') : 'Tiada' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Kaedah Pembayaran') }}</p>
            ...
            <!-- Refund Information -->
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Maklumat Bayaran Balik') }}</h3>
                <div class="space-y-4">
                                    <div>
                    <p class="text-sm text-gray-600">{{ __('Jenis Bayaran Balik') }}</p>
                    <p class="font-medium text-gray-900">{{ __('Pulangan & Bayaran Balik') }}</p>
                </div>
                    
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Sebab Bayaran Balik') }}</p>
                        <p class="text-gray-900 bg-gray-50 p-3 rounded-lg">{{ $refund->refund_reason }}</p>
                    </div>
                    
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Jumlah Bayaran Balik') }}</p>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
                <h3 class="text-lg font-semibold text-blue-900 mb-4">{{ __('Maklumat Bank untuk Bayaran Balik') }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
            ...
            @endif
             ...
             @if($refund->receipt_image)
                 <div class="bg-white rounded-lg shadow p-6">
                     <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Resit Transaksi Bayaran Balik') }}</h3>
                     <div class="text-center">
                         <a href="{{ $refund->receipt_image_url }}" 
                            target="_blank" 
                            class="block group cursor-pointer">
                             <img src="{{ $refund->receipt_image_url }}" 
                                  alt="{{ __('Resit Transaksi Bayaran Balik') }}" 
                             ...
                             <p class="text-sm text-gray-600 mt-2 group-hover:text-blue-600 transition-colors duration-200">{{ __('Resit transaksi refund telah diproses') }}</p>
                         </a>
                     </div>
                 </div>
             @endif
        ...
        <div class="space-y-6">
            <!-- Status Update Form -->
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Kemas Kini Status') }}</h3>
                    ...
                    <div>
                        <label for="status" class="block text-sm font-medium text-gray-700 mb-2">
                            {{ trans('admin_page.new_status') }} <span class="text-red-500">*</span>
                        </label>
                        <select id="status" name="status" required 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">
                            <option value="pending" {{ $refund->status === 'pending' ? 'selected' : '' }}>{{ __('Menunggu') }}</option>
                            <option value="approved" {{ $refund->status === 'approved' ? 'selected' : '' }}>{{ __('Diluluskan') }}</option>
                         ...
                         <textarea id="admin_notes" name="admin_notes" rows="3"
                                   placeholder="{{ __('Nota untuk pengguna (opsional)') }}"
                     ...
                     <div id="receipt_image_field" class="hidden">
                         <label for="receipt_image" class="block text-sm font-medium text-gray-700 mb-2">
                             Resit Transaksi Bayaran Balik <span class="text-red-500">*</span>
                         </label>
                         <input type="file" id="receipt_image" name="receipt_image" accept="image/*"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">
                         <p class="mt-1 text-sm text-gray-500">{{ __('Format: JPEG, PNG, JPG. Maksimum: 2MB') }}</p>
                         @error('receipt_image')
                    ...
                    <div id="rejection_reason_field" class="hidden">
                        ...
                        <textarea id="rejection_reason" name="rejection_reason" rows="3"
                                  placeholder="{{ __('Terangkan sebab penolakan') }}"
            ...
            <!-- User Information -->
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Maklumat Pengguna') }}</h3>
                <div class="space-y-3">
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Nama') }}</p>
                        <p class="font-medium text-gray-900">{{ $refund->user->name }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Emel') }}</p>
                        <p class="font-medium text-gray-900">{{ $refund->user->email }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Telefon') }}</p>
                        <p class="font-medium text-gray-900">{{ $refund->user->phone ?? 'Tiada' }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">{{ __('Tarikh Daftar') }}</p>
            ...
            <!-- Status History -->
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Sejarah Status') }}</h3>
                <div class="space-y-3">
                    <div class="flex items-center space-x-3">
                                ...
                                <p class="text-sm font-medium text-gray-900">
                                    @switch($refund->status)
                                        ...
                                        @case('completed')
                                            ...
                                            @break
                                        @default
                                            {{ trans('admin_page.status_changed') }}
                                    @endswitch
                                </p>
