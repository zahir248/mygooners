...
<div class="px-4 sm:px-6 lg:px-8 py-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Pengurusan Pengguna') }}</h1>
            <p class="mt-2 text-sm text-gray-700">{{ __('Urus ahli komuniti dan kebenaran mereka') }}</p>
        </div>
...
<div class="mx-4 bg-white shadow rounded-lg mb-6">
    <form method="GET" action="{{ route('admin.users.index') }}" class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <h3 class="text-lg font-medium text-gray-900 mb-4 sm:mb-0">{{ __('Tapis Pengguna') }}</h3>
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <select name="role" class="border border-gray-300 rounded-md px-3 py-2 focus:ring-red-500 focus:border-red-500 sm:text-sm">
                    ...
                    <option value="user" {{ request('role') === 'user' ? 'selected' : '' }}>{{ __('Pengguna') }}</option>
                    <option value="writer" {{ request('role') === 'writer' ? 'selected' : '' }}>{{ __('Penulis') }}</option>
                    <option value="admin" {{ request('role') === 'admin' ? 'selected' : '' }}>{{ __('Pentadbir') }}</option>
                    <option value="super_admin" {{ request('role') === 'super_admin' ? 'selected' : '' }}>{{ __('Pentadbir Utama') }}</option>
                </select>
                <!-- Status Filter -->
                <select name="status" class="border border-gray-300 rounded-md px-3 py-2 focus:ring-red-500 focus:border-red-500 sm:text-sm">
                    ...
                    <option value="suspended" {{ request('status') === 'suspended' ? 'selected' : '' }}>{{ __('Digantung') }}</option>
                </select>
                <!-- Verified Filter -->
                <select name="verified" class="border border-gray-300 rounded-md px-3 py-2 focus:ring-red-500 focus:border-red-500 sm:text-sm">
                    ...
                    <option value="1" {{ request('verified') === '1' ? 'selected' : '' }}>{{ __('Disahkan') }}</option>
                    <option value="0" {{ request('verified') === '0' ? 'selected' : '' }}>{{ __('Tidak Disahkan') }}</option>
                </select>
                <!-- Filter Buttons -->
    ...
    <div class="mx-4 mb-2">
        <div class="flex items-center justify-between">
            <div class="flex flex-wrap items-center gap-2 text-sm">
                <span class="text-gray-700 font-medium">{{ __('Tapisan aktif:') }}</span>
                @if(request('search'))
                ...
                @if(request('status'))
                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        @php
                            $statusLabels = ['active' => 'Aktif', 'suspended' => 'Digantung', 'pending' => 'Menunggu', 'inactive' => 'Tidak Aktif'];
                        @endphp
                        Status: {{ $statusLabels[request('status')] ?? ucfirst(request('status')) }}
                    </span>
                @endif
                @if(request('verified'))
...
<div class="mx-4 bg-white shadow overflow-hidden sm:rounded-lg">
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <h3 class="text-lg font-medium text-gray-900">
                {{ trans('admin_page.list_users', ['count' => $users->total()]) }}
            </h3>
        </div>
    </div>
    @if($users->count() > 0)
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Peranan') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Status') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Disahkan') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Log Masuk Terakhir') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Menyertai') }}</th>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-3">
                                    ...
                                    @if(!$user->is_verified && $user->role !== 'super_admin')
                                        <button type="button" onclick="openActionModal('verify', {{ $user->id }}, '{{ $user->name }}', '{{ route('admin.users.verify', [':id']) }}')" class="text-green-600 hover:text-green-800 btn-verify" title="{{ __('Sahkan') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if($user->status === 'active' && $user->role !== 'super_admin')
                                        <button type="button" onclick="openActionModal('suspend', {{ $user->id }}, '{{ $user->name }}', '{{ route('admin.users.suspend', [':id']) }}')" class="text-yellow-600 hover:text-yellow-800 btn-suspend" title="{{ __('Gantung') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 12H6"></path>
                                            </svg>
                                        </button>
                                    @elseif($user->status === 'suspended')
                                        <button type="button" onclick="openActionModal('activate', {{ $user->id }}, '{{ $user->name }}', '{{ route('admin.users.activate', [':id']) }}')" class="text-blue-600 hover:text-blue-800 btn-activate" title="{{ __('Aktifkan') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if(auth()->user()->role === 'super_admin' && $user->role !== 'super_admin' && $user->id !== auth()->id())
                                        <button type="button" onclick="openActionModal('delete', {{ $user->id }}, '{{ $user->name }}', '{{ route('admin.users.destroy', [':id']) }}')" class="text-red-600 hover:text-red-800 btn-delete" title="{{ __('Padam') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @else
        <div class="px-6 py-12 text-center">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('Tiada pengguna') }}</h3>
            <p class="mt-1 text-sm text-gray-500">{{ __('Tiada pengguna mengikut tapisan semasa.') }}</p>
        </div>
    @endif
</div>
...
<div id="actionModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            ...
            <h3 id="actionModalTitle" class="text-lg font-medium text-gray-900 mt-2">{{ __('Sahkan Tindakan') }}</h3>
...
<script>
    const userActionI18n = {
        verifyTitle: @json(__('Sahkan Pengguna')),
        verifyMessage: @json(__('Adakah anda pasti mahu mengesahkan pengguna ":name"?')),
        verifyConfirm: @json(__('Sahkan')),
        suspendTitle: @json(__('Gantung Pengguna')),
        suspendMessage: @json(__('Adakah anda pasti mahu menggantung pengguna ":name"?')),
        suspendConfirm: @json(__('Gantung')),
        activateTitle: @json(__('Aktifkan Pengguna')),
        activateMessage: @json(__('Adakah anda pasti mahu mengaktifkan pengguna ":name"?')),
        activateConfirm: @json(__('Aktifkan')),
        deleteTitle: @json(__('Padam Pengguna')),
        deleteMessage: @json(__('Adakah anda pasti mahu memadamkan pengguna ":name"? Tindakan ini tidak boleh diundur dan akan memadam semua data dan perkhidmatan mereka.')),
        deleteConfirm: @json(__('Padam')),
    };
    ...
    function openActionModal(type, userId, userName, route) {
        ...
        if (type === 'verify') {
            title.textContent = userActionI18n.verifyTitle;
            message.textContent = userActionI18n.verifyMessage.replace(':name', userName);
            ...
            confirmBtn.textContent = userActionI18n.verifyConfirm;
            ...
            title.textContent = userActionI18n.suspendTitle;
            message.textContent = userActionI18n.suspendMessage.replace(':name', userName);
            ...
            confirmBtn.textContent = userActionI18n.suspendConfirm;
            ...
            title.textContent = userActionI18n.activateTitle;
            message.textContent = userActionI18n.activateMessage.replace(':name', userName);
            ...
            confirmBtn.textContent = userActionI18n.activateConfirm;
            ...
            title.textContent = userActionI18n.deleteTitle;
            message.textContent = userActionI18n.deleteMessage.replace(':name', userName);
            ...
            confirmBtn.textContent = userActionI18n.deleteConfirm;
