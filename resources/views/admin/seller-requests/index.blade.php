...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">{{ __('Senarai Penjual') }}</h1>
        <div class="flex space-x-3">
            <a href="{{ route('admin.seller-requests.pending') }}" 
               class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                ...
                {{ trans('admin_page.pending_sellers', ['count' => \App\Models\User::where('seller_status', 'pending')->count()]) }}
            </a>
        </div>
    </div>
    ...
    <div class="mx-4 bg-white shadow rounded-lg mb-6">
        <form method="GET" action="{{ route('admin.seller-requests.index') }}" class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h3 class="text-lg font-medium text-gray-900 mb-4 sm:mb-0">{{ __('Tapis Senarai Penjual') }}</h3>
                <div class="flex flex-col sm:flex-row gap-4">
                    <!-- Search -->
                    <div class="relative">
                        <input type="text" 
                               name="search" 
                               placeholder="{{ __('Cari penjual...') }}"
                    ...
                    <select name="seller_status" class="border border-gray-300 rounded-md px-3 py-2 focus:ring-red-500 focus:border-red-500 sm:text-sm">
                        ...
                        <option value="approved" {{ request('seller_status') == 'approved' ? 'selected' : '' }}>{{ __('Diluluskan') }}</option>
                        <option value="rejected" {{ request('seller_status') == 'rejected' ? 'selected' : '' }}>{{ __('Ditolak') }}</option>
                    </select>
                    <!-- Filter Buttons -->
    ...
    <div class="bg-white shadow-sm rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">{{ __('Senarai Penjual') }}</h3>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Skor Kepercayaan') }}</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    @if($seller->profile_image)
                                        ...
                                        @else
                                            <img class="h-10 w-10 rounded-full object-cover" src="{{ route('profile.image', basename($seller->profile_image)) }}" alt="{{ $seller->name }}">
                                        @endif
                                    @else
                            ...
                            <div class="flex justify-end space-x-3">
                                ...
                                @if($seller->services_count > 0)
                                    <button type="button" 
                                            ...
                                            class="text-blue-600 hover:text-blue-900" title="{{ __('Lihat Perkhidmatan') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @endif
                                <button type="button" onclick="openDeleteModal({{ $seller->id }}, '{{ $seller->name }}')" class="text-red-600 hover:text-red-900" title="{{ __('Padam') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-4 text-center text-gray-500">
                            {{ __('Tiada permohonan penjual dijumpai.') }}
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="relative top-10 mx-auto p-5 border w-4/5 max-w-4xl shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium text-gray-900">{{ __('Perkhidmatan oleh') }}<span id="servicesSellerName" class="font-medium"></span></h3>
            ...
            <div id="servicesModalContent" class="max-h-96 overflow-y-auto">
                ...
                <div class="text-center py-8">
                    ...
                    <p class="mt-2 text-sm text-gray-500">{{ __('Memuatkan perkhidmatan...') }}</p>
                </div>
            </div>
        </div>
    </div>
            ...
            if (data.services && data.services.length > 0) {
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Perkhidmatan') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Penerangan') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Harga') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Status') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Tarikh Dibuat') }}</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                `;
                ...
                content.innerHTML = `
                    <div class="text-center py-8">
                        ...
                        <p class="mt-2 text-sm text-gray-500">{{ __('Tiada perkhidmatan dijumpai') }}</p>
                    </div>
                `;
            }
        })
