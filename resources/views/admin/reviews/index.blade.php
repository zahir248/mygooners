...
@section('title', __('Pengurusan Ulasan Produk'))
...
<div class="px-4 sm:px-6 lg:px-8 py-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Pengurusan Ulasan Produk') }}</h1>
            <p class="mt-2 text-sm text-gray-700">{{ __('Urus semua ulasan produk Arsenal yang diterima') }}</p>
        </div>
...
<div class="mx-4 bg-white shadow rounded-lg mb-6">
    <form method="GET" action="{{ route('admin.reviews.index') }}" class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <!-- Product Filter -->
                <select name="product_id" class="border border-gray-300 rounded-md px-3 py-2 focus:ring-red-500 focus:border-red-500 sm:text-sm">
...
<div class="mx-4 bg-white shadow overflow-hidden sm:rounded-lg">
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            @if(request('search') || request('product_id') || request('rating'))
                        ...
                        @if(request('product_id'))
                            ...
                            @if($selectedProduct)
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 mr-1">
                                    Produk: {{ Str::limit($selectedProduct->title, 20) }}
                                </span>
                            @endif
                        @endif
                        @if(request('rating'))
                                ...
                                <div class="flex items-start space-x-3">
                                    <div class="flex-shrink-0 h-16 w-24">
                                        @if($review->product->images && count($review->product->images) > 0)
                                            <img class="h-16 w-24 rounded-lg object-cover" src="{{ route('product.image', basename($review->product->images[0])) }}" alt="{{ $review->product->title }}">
                                        @else
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-sm text-gray-500 line-clamp-2">
                                            {{ trans('admin_page.list_review_product_meta', ['id' => $review->product->id, 'category' => $review->product->category]) }}
                                        </p>
                                    </div>
                                </div>
                            </td>
    ...
    @else
        <div class="px-6 py-12 text-center">
            ...
            <p class="mt-1 text-sm text-gray-500">
                @if(request('product_id') || request('rating') || request('search'))
                    Tiada ulasan dijumpai yang sepadan dengan kriteria tapisan anda.
                    @if(request('product_id'))
                        <br><span class="text-red-600">Produk ID: {{ request('product_id') }}</span>
                    @endif
                    @if(request('rating'))
