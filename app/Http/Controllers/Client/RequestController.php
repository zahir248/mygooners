...
class RequestController extends Controller
{
    ...
    public function storeServiceEditRequest(Request $request, $id)
    {
        ...
        $existingUpdateRequest = Service::where('original_service_id', $service->id)
            ->where('is_update_request', true)
            ->whereIn('status', ['pending', 'rejected'])
            ->first();

        if ($existingUpdateRequest) {
            if ($existingUpdateRequest->status === 'pending') {
                return redirect()->back()
                ...
                return redirect()->back()
                    ->with('error', 'Permohonan kemaskini untuk perkhidmatan ini telah ditolak. Sila semak sebab penolakan dan buat permohonan baharu.')
                    ->withInput();
            }
        }
